...
- `ui/swipe/SwipeScreen.kt` — drag gesture handling with `detectDragGestures`, `Animatable` for fly-off animation, threshold-based direction detection (120dp). Detects all 8 directions (4 cardinals + 4 diagonals) via the `offsetX/offsetY` ratio in `detectSwipeDirection` (cardinal vs. diagonal split at tan(67.5°) ≈ 2.414). Each direction has its own color-coded gradient overlay that fades in as the user drags; diagonal overlays use `Modifier.drawBehind` so the gradient endpoints get the actual canvas size (not `Float.MAX_VALUE`, which doesn't render reliably)
